...
Similar results were achieved with AskISC, which is using a "Retrieve and Route" scenario to route to RAG-based routes, and their solution is live in production.  Contacts:  [Sam Anthony](https://w3.ibm.com/#/people/7J9312897) and [David Elizondo-Alvarez](https://w3.ibm.com/#/people/7J8960897).

## Topic Extraction

The `TopicExtractionMiddleware` is a build time component that generates a list of topics from documents using a `TopicExtractor` and stores them in a `TopicSink`. A topic is represented by the `TopicInfo` object that has the following properties:
...
* [`altk.pre_llm.routing_toolkit.retrieval_augmented_thinking.chroma.topic_sink_content_provider.ChromaDBProvider`](#chromadbprovider-1). Stores the topics in a ChromaDB instance
...
The LLM Topic Extractor uses the [ALTK's LLM Library](https://github.ibm.com/AI4BA/agent-lifecycle-toolkit/tree/main/altk/toolkit_core/llm) to call the LLM.
...
```
python -m altk.pre_llm.routing_toolkit.retrieval_augmented_thinking.topic_extractor.cli --topic_extractor llm --topic_extraction_config sap_topic_extraction.json
```
using these topic extraction settings form the `sap_topic_extraction.json` file:
```json
{
    "model_id": "ibm/granite-3-3-8b-instruct",
    "provider": "watsonx",
    ...
    "content_provider": {
        ...
        "config": {
            "collections": [
                "sap_compensation_agent",
                "sap_employee_physical_address_agent",
                "sap_employee_phone_agent",
                "sap_employee_email_agent",
                "sap_employee_immigration_agent",
                "sap_internal_mobility_agent",
                "sap_leave_manager_agent",
                "sap_org_structure_agent",
                "sap_world_data_agent",
                "sap_termination_agent",
                "sap_employee_personal_details_agent"
            ],
            "instance": {
                "db_path": "/path/to/chroma_sap"
            }
        }
    },
    "topics_sink": {
        "name": "altk.pre_llm.routing_toolkit.retrieval_augmented_thinking.chroma.topic_sink_content_provider.ChromaDBProvider",
        "config": {
            "collection": "sap_agents_topics",
            "instance": {
                "db_path": "/path/to/chroma_sap_topics"
            }
        }
    }
}
...
The topic extractor component will extract topics from the SAP agents collections that are stored in the local chroma db in `/path/to/chroma_sap`. The topics will be extracted using the LLM `ibm/granite-3-3-8b-instruct` and the level of expertise will also be set in each topic. The extracted topics are stored in the `sap_agents_topics` collection in the local chroma db in `/path/to/chroma_sap_topics`.
...
1. Define a Pydantic model that serves as a config object for the custom content provider implementation, e.g. `MyContentProviderSettings`. You can refer to the [ChromaDBProviderSettings](https://github.ibm.com/AI4BA/agent-lifecycle-toolkit/blob/865f2867728a380958baf08d59b272bb9ec8283c/altk/routing_toolkit/retrieval_augmented_thinking/chroma/topic_sink_content_provider.py#L36-L93) object as an example.
2. Create the class that implements the [`ContentProvider` protocol](https://github.ibm.com/AI4BA/agent-lifecycle-toolkit/blob/15f8f1dd6e26f6c4f7d1a3d2bdbf05fc89e3c711/altk/routing_toolkit/retrieval_augmented_thinking/core/toolkit.py#L98-L100). The class must have a method `def get_content(self) -> Iterator[str]` that provides a stream of content chunks.
...
* `full_text_search_config`: if specified the topics will be stored so they can be queried using [Milvus full text search](https://milvus.io/docs/full-text-search.md). The following settings can be specified:
  ...
  - `sparse_index_params`: dictionary with the sparse index parameters. You can look at the [default config object](https://github.ibm.com/AI4BA/agent-lifecycle-toolkit/blob/daac396cb9ecad6aed0c59f7cd0350020c0aa9ac/altk/routing_toolkit/retrieval_augmented_thinking/milvus/common.py#L19-L23) to see what settings can be set.
...
## Topic Retrieval
The topic retriever is a run time component that retrieves the topics from different vector stores using a `TopicRetriever` object.
...
print(f"{topic_retriever_ouput.topics}")
```
By default the retriever will use full text search to query the topics. If the topics were stored to be queried using semantic search you need to provide the semantic search config in the `ann_search_config` parameter:
```python
from altk.pre_llm.routing_toolkit.retrieval_augmented_thinking.core.toolkit import (
    TopicRetrievalRunInput,
    TopicRetrievalRunOutput,
...
1. Define a Pydantic model that serves as a config object, you can refer to the [`MilvusTopicRetrieverConfig` object](https://github.ibm.com/AI4BA/agent-lifecycle-toolkit/blob/29e2558bfc8b0c5338b1509f25489dc626ec5270/altk/routing_toolkit/retrieval_augmented_thinking/milvus/topic_retriever.py#L22-L24) as an example
2. The class must have a class method that receives the config object and creates an instance of the topic retriever
   ...
   ```
3. The class must implement the [`TopicRetriever` protocol](https://github.ibm.com/AI4BA/agent-lifecycle-toolkit/blob/15f8f1dd6e26f6c4f7d1a3d2bdbf05fc89e3c711/altk/routing_toolkit/retrieval_augmented_thinking/core/toolkit.py#L121-L129) that requires a method `def get_topics(self, query: str, n_results: int = 10) -> List[TopicInfo]` that returns the topics for a user utterance
